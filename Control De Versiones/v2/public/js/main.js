...
async function loadMyProfile() {
  ...
  try {
    // Cargar stats reales del perfil
    const r = await fetch(RUTA_URL + 'perfil/mis_prompts');
    const d = await r.json();
    const prompts = d.prompts || [];

    // Actualizar contadores desde top_creadores si disponible
    if (window.PV_TOP_CREADORES) {
      const me = window.PV_TOP_CREADORES.find(u => u.id === window.PV_USER.id);
      if (me) {
        ...
        if (mpP) mpP.textContent = me.num_prompts || 0;
        if (mpS) mpS.textContent = formatNumberJS(me.num_seguidores) || 0;
        if (mpSi) mpSi.textContent = me.num_siguiendo || 0;
      }
    }
    // Si no está en top, al menos mostramos count de prompts
    const mpP = document.getElementById('mp-prompts');
    if (mpP && mpP.textContent === '—') mpP.textContent = prompts.length;
