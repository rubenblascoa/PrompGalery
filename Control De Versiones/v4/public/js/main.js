...
function goPage(page) {
  ...
  if (page === 'colecciones') { window.location.href = RUTA_URL + 'colecciones'; return; }
...
function openModal(type) {
  ...
  if (type === 'editperfil' && window.PV_USER) {
    ...
    const fillEditForm = (perfil) => {
      if (!perfil) return;
      if (bioEl)    { bioEl.value = perfil.bio || ''; updateCharCount(bioEl, 'edit-bio-count', 300); }
      if (ciudadEl) ciudadEl.value = perfil.ciudad || '';
      if (webEl)    webEl.value = perfil.sitio_web || '';
    };

    if (window.PV_PERFIL) {
      fillEditForm(window.PV_PERFIL);
    } else {
      // PV_PERFIL null (usuario no ha visitado su perfil en esta sesión) → fetch ahora
      fetch(RUTA_URL + 'perfil/stats', { credentials: 'same-origin' })
        .then(r => r.ok ? r.json() : null)
        .then(d => {
          if (d && d.stats) {
            window.PV_PERFIL = d.stats;
            fillEditForm(window.PV_PERFIL);
          }
        })
        .catch(() => {});
...
// HELPERS
// ═══════════════════════════════════════════════════════════
function openPromptDetailSPA(id) {
  if (typeof loadPromptDetail === 'function') {
    loadPromptDetail(id);
    openModal('prompt');
  } else {
  ...
  searchTimeout = setTimeout(async () => {
    try {
      ...
      if (!d.resultados.length) {
        ...
        const shown = d.resultados.slice(0, 6);
        const total  = d.total || shown.length;
        results.innerHTML = shown.map(p => `
          <div class="search-result-item" onclick="results.classList.remove('open');openPromptDetailSPA('${p.id}')">
            <div class="sri-title">${escHtml(p.titulo)}</div>
            <div class="sri-meta">${escHtml(p.categoria)} · @${escHtml(p.usuario_nombre||'')} · ▲${p.votos_positivos||p.votos_pos||0}</div>
          </div>
        `).join('') + (total > 6 ? `<a class="search-result-item" href="${RUTA_URL}explorar?q=${encodeURIComponent(val)}" style="text-align:center;color:var(--accent);font-size:13px;text-decoration:none">Ver los ${total} resultados →</a>` : '');
      }
...
async function profileTab(el, tab) {
  ...
  if (tab === 'saved') {
    ...
    window.location.href = RUTA_URL + 'colecciones';
    return;
  } else if (tab === 'activity') {
    content.innerHTML = `<div style="text-align:center;padding:32px;color:var(--text3)"><div class="typing-indicator" style="justify-content:center"><div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div></div><div style="margin-top:12px;font-size:13px">Cargando actividad...</div></div>`;
    try {
      const r = await fetch(RUTA_URL + 'perfil/actividad');
      const d = await r.json();
      const acts = d.actividad || [];
      if (!acts.length) {
        content.innerHTML = `<div class="empty-state"><div class="empty-icon">📊</div><div class="empty-title">Sin actividad reciente</div><div class="empty-sub">Tu actividad aparecerá aquí</div></div>`;
      } else {
        const iconMap = { prompt_creado: '✨', comentario_dado: '💬', voto_dado: '⬆️' };
        const labelMap = {
          prompt_creado:   a => `Publicaste <strong>"${escHtml((a.ref_titulo||'').substring(0,50))}"</strong>`,
          comentario_dado: a => `Comentaste en <strong>"${escHtml((a.ref_titulo||'').substring(0,50))}"</strong>`,
          voto_dado:       a => `Votaste <strong>"${escHtml((a.ref_titulo||'').substring(0,50))}"</strong> de @${escHtml(a.actor_nombre||'')}`,
        };
        content.innerHTML = '<div style="display:flex;flex-direction:column;gap:10px">' + acts.map(a => `
          <div style="display:flex;gap:12px;align-items:flex-start;padding:12px;background:var(--surface);border-radius:10px;border:1px solid var(--border)">
            <div style="font-size:20px;flex-shrink:0;width:32px;text-align:center;margin-top:2px">${iconMap[a.tipo]||'📌'}</div>
            <div style="flex:1;min-width:0">
              <div style="font-size:13px;color:var(--text1);line-height:1.5">${(labelMap[a.tipo]||(()=>'Acción'))(a)}</div>
              <div style="font-size:11px;color:var(--text3);margin-top:4px">${timeAgoJS(a.fecha)}</div>
            </div>
          </div>`).join('') + '</div>';
      }
    } catch {
      content.innerHTML = `<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-title">Error al cargar actividad</div></div>`;
    }
  } else {
...
function toggleNotifications() {
  ...
  if (panel?.classList.contains('open')) {
    ...
    setTimeout(() => {
      ...
      fetch(RUTA_URL + 'perfil/marcar_notif_leidas', { method: 'POST', body: fd }).catch(() => {});
    }, 1000);
  }
}
...
async function markAllRead() {
  ...
  try {
    const fd = new FormData();
    fd.append('csrf_token', CSRF());
    await fetch(RUTA_URL + 'perfil/marcar_notif_leidas', { method: 'POST', body: fd });
  } catch {}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // colecciones tiene su propia URL (/colecciones)
